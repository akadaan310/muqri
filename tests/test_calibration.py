...
from app.calibration import Calibration, MetricBand, rule_key, verdict
...
@pytest.mark.parametrize("metrics,reason", [
    ({"align_conf": 0.1, "align_min_ms": 200, "core_ms": 300, "core_counts": 0.2}, "posterior"),
...
def test_unreliable_alignment_is_skipped_not_failed(metrics: dict[str, float], reason: str) -> None:
    d = CAL.apply(diag(RuleType.MADD_TABII, **metrics))
    assert d.status is Status.SKIPPED and d.score is None and reason in d.feedback
